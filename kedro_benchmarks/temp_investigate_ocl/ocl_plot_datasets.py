...
def generate_variable_interpolation_catalog(start_idx, num_vars):
    """Creates a catalog where each dataset"""
    catalog = {}
    ...
    catalog_path = base_path / f"catalog_datasets.yml"
...
def run_scaling_tests():
    ...
    start_idx = 0
    for num_datasets in [10, 100, 500, 1000, 2000, 5000, 8000, 10000]:
        catalog_path = generate_variable_interpolation_catalog(start_idx, num_datasets)
        load_time, config_len = load_and_time_catalog(catalog_path)
        results.append(
            (
                f"Catalog with {num_datasets} datasets without variable interpolation",
                num_datasets,
                load_time,
                config_len,
            )
        )
        start_idx += num_datasets
...
def plot_results(results):
    ...
    plt.title("OmegaConfigLoader Scaling Properties without Variable Interpolations")
